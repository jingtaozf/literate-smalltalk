...
(define-quick-choice (literate-smalltalk-code-mode)
  ...
  ("compile codes" literate-smalltalk-namespace-of-current-symbol)
  ("update codes" literate-smalltalk-namespace-of-current-symbol)
  )
#+END_SRC
...
baselineOf: spec
	...
	spec
		...
		do: [
			...
			spec package: 'LiterateSmalltalk' with: [ spec requires: #('Teapot') ].
...
start
	...
	| teapot |
        ...
        " self server ifNotNil: [ setf server stop]."
...
compileClass: instDef class: classDef
    ...
    status := #success.
    instvars := ''.
    (instDef at: #instvars) ifNotNil: [ instvars := (self elementsString: (instDef at: #instvars)) ].

    classvars := ''.
    (instDef at: #classvars) ifNotNil: [ classvars := (self elementsString: (instDef at: #classvars)) ].
    ...
    instvars := ''.
    (classDef at: #instvars) ifNotNil: [ instvars := (self elementsString: (classDef at: #instvars)) ].
